...
// == command Ebi itself python == 

	public static String Ebi_itself_python__as__string__to__string(PluginContext context) throws Exception {
		String result = CallEbi.call_ebi("Ebi itself python", ".txt", new String[] {});
		return org.processmining.ebi.objects.EbiString.fromEbiString(context, result);
	}

	@Plugin(
		name = "Generate the module exposed to Python with all functions. (input: ; output: string)",
		level = PluginLevel.PeerReviewed, 
		returnLabels = { "string" }, 
		returnTypes = { String.class },
		parameterLabels = { "" },
		userAccessible = true,
		categories = { PluginCategory.Discovery, PluginCategory.Analytics, PluginCategory.ConformanceChecking },
		help = "Generate the module exposed to Python with all functions. (calls Ebi)"
	)
	@UITopiaVariant(affiliation = IMMiningDialog.affiliation, author = IMMiningDialog.author, email = IMMiningDialog.email)
	@PluginVariant(variantLabel = "Call Ebi", requiredParameterLabels = {  })
	public String prom_Ebi_itself_python__as__string__to__string(PluginContext context) throws Exception {
		return Ebi_itself_python__as__string__to__string(context);
	}
